# /Users/anirudhravipudi/Desktop/Portfolio/Healthcare/CLinicalExplorer/app.py
...
# -------- Paths --------
BASE = Path("/Users/anirudhravipudi/Desktop/Portfolio/Healthcare")
DATA = BASE / "data"
CURATED_CSV = DATA / "clinical_notes_curated.csv"
...
def infer_sex(text: str) -> str:
    if not text:
        return "Unknown"
    t = " " + text.lower() + " "  # pad for token matches like " mr "
    # priority: explicit "female/male" beats pronouns
    if " female " in t:
        return "Female"
    if " male " in t:
        return "Male"
...
def load_notes():
    if not CURATED_CSV.exists():
        raise FileNotFoundError(f"Missing file: {CURATED_CSV}")
    ...
    source_cols = [c for c in ["visit_motivation","main_symptom","note","full_note","conversation","summary"]
                   if c in df.columns]
    if not source_cols:
        # last resort: create empty note so app stays functional
        df["note"] = df.get("note", pd.Series([""] * len(df)))
        source_cols = ["note"]

    df["text_all"] = df[source_cols].fillna("").agg(" ".join, axis=1).str.replace(r"\s+", " ", regex=True).str.strip()
...
# Keyword search (commas = OR; AND mode also available)
...
# Age (only if any present)
age_min = int(df.loc[df["has_age"], "age_years"].min()) if df["has_age"].any() else 0
age_max = int(df.loc[df["has_age"], "age_years"].max()) if df["has_age"].any() else 100
if df["has_age"].any():
    sel_age = st.sidebar.slider("Age (years)", age_min, age_max, (age_min, age_max))
else:
    sel_age = None

# Sex choices (include Unknown)
...
if sel_age is not None:
    view = view[df["has_age"] & df["age_years"].between(sel_age[0], sel_age[1])]
...
with tab1:
    ...
    with c2:
        ...
        if view["has_age"].any():
            fig_age = px.histogram(view[view["has_age"]], x="age_years", nbins=30,
...
with tab2:
    ...
    if "age_years" in view.columns: cols_to_show.append("age_years")

    st.dataframe(view[cols_to_show].iloc[start:end].reset_index(drop=True),
    ...
    if idxs:
        ...
        with st.expander("Full text", expanded=True):
            show_cols = [c for c in ["visit_motivation","main_symptom","note","full_note","conversation","summary"] if c in view.columns]
            if not show_cols: show_cols = ["text_all"]
            for c in show_cols:
                st.markdown(f"**{c}**")
                st.write(rec.get(c, ""))
...
with tab3:
    ...
    if model is None:
        ...
        sample = ("Patient presents with intermittent chest discomfort and shortness of breath. "
                  "ECG ordered. Started on low-dose aspirin.")
        txt = st.text_area("Paste a note:", height=180, value="")
        ...
        if st.button("Predict") and txt.strip():
            ...
            top = pd.DataFrame({"category": cls[order][:5],
                                "probability": (probs[order][:5]*100).round(2)})
            st.success(f"Prediction: **{cls[order][0]}**")
            figp = px.bar(top, x="category", y="probability", text="probability",
